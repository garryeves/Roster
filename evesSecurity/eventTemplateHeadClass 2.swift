...
public class eventTemplateHeads: NSObject, Identifiable
{
    ...
    {
        if currentUser.currentTeam?.eventTemplateHeads == nil
        {
            currentUser.currentTeam?.eventTemplateHeads = myCloudDB.getEventTemplateHeadItems(teamID: teamID)
        }
        
        for myItem in (currentUser.currentTeam?.eventTemplateHeads)!
        {
...
public class eventTemplateHead: NSObject, Identifiable
{
    ...
    public init(eventID: Int64, teamID: Int64)
    {
        ...
        for item in (currentUser.currentTeam?.eventTemplateHeads)!
        {
